...
void ModbusController::initialize() {
    ...
    //ModbusDevice.connectToDevice(m_ip_address_, m_modbus_port_);
}
...
void ModbusController::writeCommands(writeParameters &writeParams) {
    ...
    if (initiateTopModule) {
        ...
        // Changed from RCLCPP_DEBUG_NAMED to RCLCPP_DEBUG
    ...
    // Write to modbus
    m_modbus_device_.writeData(STARTINGINDEX, WRITEREGISTERNUMBER, write_reg_data_);
}

void ModbusController::readCommands(readParameters &readParam) {
    if (!m_modbus_device_.readData(0, READREGISTERNUMBER)) {
        RCLCPP_ERROR(node_->get_logger(), "Failed to read modbus data");
        return;
    }
    ...
    // Read motor speeds
    readParam.m_leftMotorSpeed = static_cast<int16_t>(m_modbus_device_.m_registerData[m_left_motor_speed_pin_].to_ulong());
    readParam.m_rightMotorSpeed = static_cast<int16_t>(m_modbus_device_.m_registerData[m_right_motor_speed_pin_].to_ulong());
