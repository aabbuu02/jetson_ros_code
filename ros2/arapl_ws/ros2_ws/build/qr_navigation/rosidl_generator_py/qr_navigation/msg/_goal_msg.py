...
class Metaclass_GoalMsg(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            module = import_type_support('qr_navigation')
        except ImportError:
            import logging
            import traceback
            logger = logging.getLogger(
                'qr_navigation.msg.GoalMsg')
            logger.debug(
...
class GoalMsg(metaclass=Metaclass_GoalMsg):
    ...
    def __init__(self, **kwargs):
        ...
        self.start = array.array('d', kwargs.get('start', []))
    ...
    def start(self, value):
        if isinstance(value, array.array):
            assert value.typecode == 'd', \
                "The 'start' array.array() must have the type code of 'd'"
            self._start = value
            return
        if __debug__:
            ...
            assert \
                ...
                "The 'start' field must be a set or sequence and each value of type 'float' and each double in [-.000000, .000000]"
        self._start = array.array('d', value)
